...
import assignment_2_2023.msg # original msg
from tf import transformations
from std_srvs.srv import * # standard service of ros
import time
...
regions_ = None # initialize the LaserScan data
...
# function that deal with odometry msg from /odom topic
# renew the position and yaw angle
def clbk_odom(msg):
...
# function that deal with laserScan msg from /scan topic
# renew the data of distance from surrounding the robot
def clbk_laser(msg):
    global regions_
    regions_ = {
...
# function that renew the state of the robot
# call the services in accordance to the state
...
def planning(goal):
    # initialize the state of robot to start the action
    ...
    # loop until robot reach the goal or the action is cancelled
	...
	# act in accordance to the state
        elif state_ == 0:
...
# initialize the ROS nodes amd set the service clients and action servers
def main():
    time.sleep(2)
    global regions_, position_, desired_position_, state_, yaw_, yaw_error_allowed_
